    ...
    <form action="{{ route('vendas.update', $venda->id) }}" method="POST" id="form-venda">
        ...
        <div class="mb-4">
            ...
            <select name="cliente_id" id="cliente_id" class="form-select" required>
                <option value="{{ $venda->cliente->id }}" selected>{{ $venda->cliente->nome }}</option>
            </select>
        </div>
            ...
            <tbody>
                @foreach ($venda->produtos as $produto)
                    <tr data-produto-id="{{ $produto->id }}">
                        ...
                        <td>
                            @if ($produto->imagem)
                                <img src="{{ asset($produto->imagem) }}" alt="{{ $produto->nome }}" width="40" class="me-2 rounded">
                            @endif
                        </td>
                        <td>R$ {{ number_format($produto->preco, 2, ',', '.') }}</td>
                        <td>
                            <input type="number" name="produtos[{{ $produto->id }}][quantidade]" value="{{ $produto->pivot->quantidade }}" min="1" max="{{ $produto->estoque }}" class="form-control quantidade">
                            <input type="hidden" name="produtos[{{ $produto->id }}][id]" value="{{ $produto->id }}">
                        </td>
                        <td class="total-produto">R$ {{ number_format($produto->preco * $produto->pivot->quantidade, 2, ',', '.') }}</td>
                        <td>
                            <button type="button" class="btn btn-sm btn-danger remover-produto"><i class="bi bi-trash"></i></button>
                        </td>
                    </tr>
                @endforeach
            </tbody>
        ...
        <button type="submit" class="btn btn-primary mt-4">
            <i class="bi bi-save me-1"></i> Atualizar Venda
        </button>
    </form>
</div>
@endsection
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Carrega produtos da venda inicial
    @foreach ($venda->produtos as $produto)
        produtosSelecionados.set({{ $produto->id }}, {
            id: {{ $produto->id }},
            text: '{{ $produto->nome }}',
            preco: {{ $produto->preco }},
    ...
    $('#produto_select').on('select2:select', function (e) {
        ...
        row.innerHTML = `
            ...
            <td>
                ${data.imagem ? `<img src="${data.imagem}" alt="${data.text}" width="40" class="me-2 rounded">` : ''}
            </td>
